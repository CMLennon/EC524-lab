...
p_load(
  ...
  huxtable, here, magrittr, tidymodels, caret, party, recipes, workflows, glmnet, C5.0, tensorflow, keras, abind, pbapply, DT, plotly, reticulate, keras
)
...
$\frac{\partial{J}}{\partial{w^t_j}} = - \sum(y^{(i)} - \phi(z)^{(i)})x^{(i)}_j$, $W^{t+1} = W^t - \eta \nabla J(W^t)$
...
<center>
<img src="https://www.doc.ic.ac.uk/~nuric/assets/img/neuron_to_layer.457ecc22.gif" alt="imperial college gif", width = "80%">
</center>
...
<center>
<img src="https://www.doc.ic.ac.uk/~nuric/assets/img/neuron_to_layer.457ecc22.gif" alt="imperial college gif", width = "30%">
</center>
...
```{r, mnist, echo = F, eval = T}
load(file = here('mnist.RData'))
#mnist <- dataset_fashion_mnist()
```
...
![Recurrent Neural Network](images/rnn.gif)
...
![lstm step 1](images/tanh-dat.gif)


---
#LSTM step 2

![lstm drop cell-state?](images/lstm-inputgate.gif)

---
#LSTM step 3

![lstm how much to add to cell-state?](images/lstm_update.gif)

---
#LSTM final

![final step](images/lstm-final-step.gif)
...
```{r, dataprep, echo = F}
hhpowerdata = fread('data/household_power_consumption.txt', sep = ';', nrows = 100)

hhpowerdata = hhpowerdata %>% head(100)

hhpowerdata[, datetime := as.POSIXct(paste(Date, Time), format="%d/%m/%Y %H:%M:%S")]

#setkey(hhpowerdata, datetime)
...
hhpowerdata[, c(numeric_cols) := lapply(.SD, as.numeric), .SDcols=numeric_cols]

for(j in numeric_cols){
  set(hhpowerdata,j=j,value = nafill(hhpowerdata[[j]], 'locf'))
}

hhpowerdata[, c(numeric_cols) := lapply(.SD, function(x) (x-min(x))/max(x)), .SDcols = numeric_cols]

```

```{r, rolling_window_array, echo = F, eval = F}
train_window = 180
assess_window = 10

modified_rolling_window_array = function(sorted_dt, train_window, assess_window, is.x = TRUE){
  dt_list_x = list()
  dt_list_y = list()
  sorted_dt = sorted_dt[,.SD,.SDcols = is.numeric]
  
  subfun = function(start_row, train_window, assess_window, is.x, sorted_dt){
    train_end = start_row + train_window
    test_end = train_end + assess_window
    
    
    if(is.x){
      return(sorted_dt[start_row:(train_end-1)])
    }
    else{
      return(sorted_dt[train_end:(test_end-1),1])
    }
  }
  
  data_out = lapply(seq(1, to = nrow(sorted_dt),by = train_window), subfun, train_window = train_window, assess_window = assess_window, is.x = is.x, sorted_dt = sorted_dt)

  
  return(abind(data_out, along = 3))
}

data_prep = modified_rolling_window_array(head(hhpowerdata, n = -18000), train_window = train_window, assess_window = assess_window)

data_prep_y = modified_rolling_window_array(sorted_dt = head(hhpowerdata, n = -18000), train_window = train_window, assess_window = assess_window, is.x = F)

data_test = modified_rolling_window_array(tail(hhpowerdata, n = 18000), train_window = train_window, assess_window = assess_window)

data_test_y = modified_rolling_window_array(tail(hhpowerdata, n = 18000), train_window = train_window, assess_window = assess_window, is.x = F)

batches = dim(data_prep)[3]
test_batches = dim(data_test)[3]
train_window = dim(data_prep)[1]
features = dim(data_prep)[2]

data_prep_tf = aperm(data_prep, c(3,1,2))[1:batches-1,,]
data_prep_y_tf = aperm(data_prep_y, c(3,1,2))[1:batches-1,,]

data_test_tf = aperm(data_test, c(3,1,2))[1:test_batches-1,,]
data_test_y_tf = aperm(data_test_y, c(3,1,2))[1:test_batches-1,,]
```

```{r,echo = F}
dispvect = c('datetime', 'Global_active_power', 'Voltage', 'Global_intensity')
DT::datatable(
  head(hhpowerdata[,..dispvect],4),
...
```{r, echo = F,eval = F}
model = load_model_hdf5("/Users/connor/Desktop/GithubProjects/Econometrics/524/EC524W20/lab/005-Perceptrons_and_NeuralNets/lstm_model_sf.hdf5")
```

```{r, echo = F, eval = F}
pred_out = model %>% predict(data_test_tf)
pred_samp = pred_out[42,]
compare = data_test_y_tf[42,]
compare2 = data_test_tf[42,,1]
startpt = length(compare2)
index = 1:190
df1 = data.frame(actual = c(compare2,compare), index = index, err = abs(compare-pred_samp))
df2 = data.frame(actual = pred_samp)

p = plot_ly(df1, x = ~index, y = ~actual, type = "scatter", mode = "lines") %>%
 add_trace(y = c(rep(NA,startpt), pred_samp), x = df1$index, name = "LSTM prediction", mode = "lines")

p
```

```{r,echo = F, eval = F}
load("/Users/connor/Desktop/GithubProjects/Econometrics/524/EC524W20/lab/005-Perceptrons_and_NeuralNets/lstm.html")
```
```{r, echo = F}
knitr::include_graphics('lstm-preds.png')
```
...
pagedown::chrome_print(input = "005-slides.html")
```
